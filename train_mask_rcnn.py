...
def get_dataset(dataset, args):
    if dataset.lower() == 'coco':
        train_dataset = gdata.COCOInstance(root='F:/HoangAnh/ScanComProject/Images/dataTrainBaseline/Lateral/dataTrain-27-5/trainFormatCoco/train',
                                            splits='annotations')
        val_dataset = gdata.COCOInstance(root='F:/HoangAnh/ScanComProject/Images/dataTrainBaseline/Lateral/dataTrain-27-5/trainFormatCoco/train',
...
def train(net, train_data, val_data, eval_metric, batch_size, ctx, logger, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        rcnn_task = ForwardBackwardTask(net, trainer, rpn_cls_loss, rpn_box_loss, rcnn_cls_loss,
                                        rcnn_box_loss, rcnn_mask_loss,amp_enabled = False)
