...
os.environ['MXNET_GPU_COPY_NTHREADS'] = '2'
...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        ...
        train_dataset = gdata.COCODetection(root='/kaggle/input/dataset/trainData/train',
                                            splits='annotations', use_crowd=False)
        val_dataset = gdata.COCODetection(root='/kaggle/input/dataset/trainData/valid',
                                            splits='annotations', skip_empty=False)
    ...
    print("train_dataset : ",dir(train_dataset))
    print("train_dataset : ",(train_dataset.annotation_dir))
...
def  get_lr_cosin(lr,warmup_steps,global_step,total_steps):
    return 0.5 * lr * (1 + np.cos(
        np.pi *
